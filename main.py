...
def load_proxies_from_file(filename="proxy.txt"):
    ...
    try:
        ...
        logger.info(f"Successfully loaded {len(proxies)} proxies from {filename}")
        return proxies
...
async def main():
    ...
    logger.info(f"Loaded {len(all_proxies)} proxies and {len(tokens)} tokens")
